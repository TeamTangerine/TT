...
function Modal({ isOpen, isClose, message, rightLabel, action }: ModalProps) {
  ...
  return createPortal(
    <>
      {/* 모달 전용 백드롭 (배경) */}
      <div className="fixed inset-0 z-40 bg-black bg-opacity-50" onClick={isClose} />

      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[252px] h-[110px] rounded-[10px] bg-white border border-[#DBDBDB] z-50 flex flex-col justify-center">
